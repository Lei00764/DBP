...
public class LikeController :ControllerBase
{
    ...
    public async Task<IActionResult> LikeArticleAsync(int user_id,int post_id)//点赞或取消点赞
    {
        ...
        //Console.Write(a);
        var record=_database.Like.Where(x => x.PostId == post_id&&x.UserId==user_id);
        bool u_exist=false;
        bool a_exist=false;
        bool r_exist=false;
        ...
        foreach(var item in a)//更改点赞数
        {
            if(r_exist==false)//未点赞
            {
                ...
                _database.Like.AddRange(newRecord);
                await _database.SaveChangesAsync();
            }
            else//已点赞
            {
                item.LikeNum-=1;
                await _database.SaveChangesAsync();
                _database.Like.RemoveRange(record);//删除记录
                await _database.SaveChangesAsync();
            }
        }
